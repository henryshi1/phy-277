...
int main() {
  ...
  double vx [4] = {0.0, 0.0, 0.0, 0.0};       // array of x-velocities (m/s)
  double vy [4] = {0.0, 0.0, 0.0, 0.0};       // array of x-velocities (m/s)
  ...
  do {
    ...
    // Increment the values of velocity
    vx[0] = vx[1];
    vx[1] = vx[2];
    vx[2] = vx[3];
    vx[3] += dvx;
...
double get_dx(double vx[], double dt) {
  double dx = (vx[0] + 2.0*vx[1] + 2.0*vx[2] + vx[3]) * dt / 6.0;
  return dx;
}

// dy = vy*dt
double get_dy(double vy[], double dt) {
  double dy = (vy[0] + 2.0*vy[1] + 2.0*vy[2] + vy[3]) * dt / 6.0;
  return dy;
}

// v = sqrt( (vx)^2+(vy)^2 )
double get_v(double vx[], double vy[]) {
  double v_x = 0.5 * (vx[2] + vx[3]);
  double v_y = 0.5 * (vy[2] + vy[3]);
...
double get_dvx(double vx[], double vy[], double dt){
  ...
  double vx0 = vx[0], vx1 = vx[1], vx2 = vx[2], vx3 = vx[3];
  double dvx = (-alpha * v * vx0 - 2.0 * alpha * v * vx1
		- 2.0 * alpha * v * vx2 - alpha * v * vx3) * dt;
  return dvx;
}
...
double get_dvy(double vx[], double vy[], double dt[]){
  double v = get_v(vx, vy);
  double vy0 = vy[0], vy1 = vy[1], vy2 = vy[2], vy3 = vy[3];  
  double dvy = (-alpha * v * vy0) - (2.0 * alpha * v * vy1)
    - (2.0 * alpha * v * vy2) - (alpha * v * vy3) - (4.0 * g);
  dvy = dvy * dt;
  return dvy;
}
